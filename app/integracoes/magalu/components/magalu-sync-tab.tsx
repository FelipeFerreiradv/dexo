...
export function MagaluSyncTab() {
  ...
  const pollImportStatus = useCallback(
    async (jobId: string) => {
      ...
      const response = await fetch(
        `${getApiBaseUrl()}/marketplace/magalu/import/${jobId}`,
      ...
      if (statusData.status === "failed") {
        ...
        setError(
          statusData.progress?.message ||
            ...
            "Erro ao importar anúncios da Magalu",
        );
      }
    },
    [session?.user?.email],
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      const url = new URL(`${getApiBaseUrl()}/marketplace/magalu/import`);
  ...
  const handleSync = useCallback(async () => {
    ...
    try {
      const url = new URL(`${getApiBaseUrl()}/marketplace/magalu/sync`);
      if (selectedAccountId)
        url.searchParams.set("accountId", selectedAccountId);
    ...
    <div className="space-y-4">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Busca seus anúncios na Magalu e tenta vincular automaticamente aos
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Envia o estoque atual dos produtos vinculados para a Magalu. Somente
          ...
          <Button onClick={handleSync} disabled={isSyncing} variant="secondary">
            {isSyncing ? (
              ...
              <>
                ...
                Sincronizar Estoque com a Magalu
              </>
            )}
          </Button>
                            ...
                            {syncResult.results.map((result) => (
                              ...
                              >
                                <div className="flex-1">
                                  ...
                                  {result.success ? (
                                    <div className="text-xs text-muted-foreground">
                                      Estoque atualizado: {result.newStock}
                                    </div>
                                  ) : (
