...
export default function MagaluCallbackPage() {
  ...
  useEffect(() => {
    const processCallback = async () => {
      // 1. Redirect do backend (result=success/error)
      ...
      if (result === "success") {
        ...
        notifyParent("MAGALU_OAUTH_SUCCESS");
        setTimeout(() => {
          window.close();
        }, 2000);
        return;
      ...
      if (result === "error") {
        ...
        notifyParent("MAGALU_OAUTH_ERROR", resultMessage || "Erro desconhecido");
        return;
      }

      // 2. Fallback: code+state direto na URL
      ...
      if (error) {
        ...
        notifyParent("MAGALU_OAUTH_ERROR", error);
        return;
      }
      ...
      if (!code || !state) {
        ...
        notifyParent("MAGALU_OAUTH_ERROR", "Parâmetros inválidos");
        return;
      }

      try {
        const response = await fetch(
          `${getApiBaseUrl()}/marketplace/magalu/callback?code=${encodeURIComponent(code)}&state=${encodeURIComponent(state)}`,
        );
  ...
  const notifyParent = (type: string, message?: string) => {
    if (
      ...
      try {
        ...
        console.warn("[Magalu Callback] Erro ao enviar postMessage:", err);
        try {
          window.opener.postMessage({ type, message }, "*");
        } catch (fallbackErr) {
          console.error(
            "[Magalu Callback] Fallback postMessage também falhou:",
            fallbackErr,
          );
        }
      }
      ...
      <div className="w-full max-w-sm rounded-lg border bg-card p-6 text-center shadow-lg">
        ...
        {status === "error" && (
          <>
            ...
            <h2 className="mt-4 text-lg font-semibold text-destructive">Erro</h2>
            <p className="mt-2 text-sm text-muted-foreground">{message}</p>
            <button
