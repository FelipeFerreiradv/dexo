...
export default function OlxCallbackPage() {
  ...
  useEffect(() => {
    const processCallback = async () => {
      ...
      if (result === "success") {
        ...
        notifyParent("OLX_OAUTH_SUCCESS");
        setTimeout(() => {
          window.close();
        }, 2000);
        return;
      ...
      if (result === "error") {
        ...
        notifyParent("OLX_OAUTH_ERROR", resultMessage || "Erro desconhecido");
        return;
      }
      ...
      if (error) {
        ...
        notifyParent("OLX_OAUTH_ERROR", error);
        return;
      }
      ...
      if (!code || !state) {
        ...
        notifyParent("OLX_OAUTH_ERROR", "Parâmetros inválidos");
        return;
      }

      try {
        const response = await fetch(
          `${getApiBaseUrl()}/marketplace/olx/callback?code=${encodeURIComponent(code)}&state=${encodeURIComponent(state)}`,
        );
  ...
  const notifyParent = (type: string, message?: string) => {
    if (
      ...
      try {
        ...
        console.warn("[OLX Callback] Erro ao enviar postMessage:", err);
        try {
          window.opener.postMessage({ type, message }, "*");
        } catch (fallbackErr) {
          console.error(
            "[OLX Callback] Fallback postMessage também falhou:",
            fallbackErr,
          );
        }
      }
