...
export function OlxSyncTab() {
  ...
  const handleSync = useCallback(async () => {
    ...
    try {
      const url = new URL(`${getApiBaseUrl()}/marketplace/olx/sync`);
  ...
  useEffect(() => {
    const loadAccounts = async () => {
      if (!session?.user?.email) return;
      try {
        const res = await fetch(`${getApiBaseUrl()}/marketplace/olx/accounts`, {
          headers: { email: session.user.email },
        });
        if (res.ok) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Nota: OLX é unidirecional (ERP→OLX) e sem importação de pedidos. */}
      <div className="flex items-start gap-2 rounded-md border border-border/60 bg-muted/30 p-3 text-xs text-muted-foreground">
        ...
        <span>
          A OLX não fornece webhook de venda nem importação de pedidos. A baixa
          de estoque é <strong>unidirecional</strong> (Dexo → OLX): quando o
          estoque zera, o anúncio é despublicado; ao repor, ele é re-publicado
          (entra na fila de revisão da OLX). Vendas feitas na OLX devem ser
          registradas manualmente como venda de balcão.
        </span>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Envia o estoque atual dos produtos vinculados para a OLX. Somente
            produtos com vínculo ativo serão sincronizados.
          </CardDescription>
        </CardHeader>
                ...
                <>
                  ...
                  {syncResult.total === 0 && (
                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                      ...
                      <span>
                        Nenhum produto vinculado encontrado. Publique anúncios
                        na OLX a partir dos seus produtos primeiro.
                      </span>
                    </div>
                  )}
                </>
