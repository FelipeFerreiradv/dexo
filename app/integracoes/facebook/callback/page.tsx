  ...
  useEffect(() => {
    const processCallback = async () => {
      ...
      if (error) {
        ...
        notifyParent("FACEBOOK_OAUTH_ERROR", error);
        return;
      }
      ...
      if (!code || !state) {
        ...
        notifyParent("FACEBOOK_OAUTH_ERROR", "Parâmetros inválidos");
        return;
      }

      try {
        const response = await fetch(
          `${getApiBaseUrl()}/marketplace/facebook/callback?code=${encodeURIComponent(code)}&state=${encodeURIComponent(state)}`,
        );
  ...
  const notifyParent = (type: string, message?: string) => {
    if (
      ...
      try {
        ...
        console.warn("[Facebook Callback] Erro ao enviar postMessage:", err);
        try {
          window.opener.postMessage({ type, message }, "*");
        } catch (fallbackErr) {
          console.error(
            "[Facebook Callback] Fallback postMessage também falhou:",
            fallbackErr,
          );
        }
      }
